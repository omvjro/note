...
const Del: React.FC<Pick<DetailedHTMLProps<DelHTMLAttributes<HTMLModElement>, HTMLModElement>, 'key' | keyof DelHTMLAttributes<HTMLModElement>> & ReactMarkdownProps> = ({ children, ...properties }) => (
  <span {...properties} className="bg-slate-50 text-slate-50 hover:text-black duration-500 dark:text-black dark:bg-neutral-900 hover:dark:text-gray-200 hover:dark:bg-neutral-900 cursor-pointer">{children}</span>
)
